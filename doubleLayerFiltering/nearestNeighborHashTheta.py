    ...
    for event in reader:
        ...
        for hit in hitsCollection:
            ...
            if layer==1:
                ...
                if noHits[0]:
                    noHits[0]=False
                pseudoRapidity=hit.getPositionVec().Theta()
            ...
            #Adding hits from the first doublet layer to a list so we can find their closest pairs later
                ...
                if noHits[1]:
                    noHits[1]=False
                firstLayerHit.append((hit.getPositionVec().Theta(),hit.getPositionVec().Phi()))
        ...
        for hit in hitsCollection:
            ...
            #All other hits are in the first layer of a doublet
                ...
                firstLayerHit[index].append((hit.getPositionVec().Theta(),hit.getPositionVec().Phi()))
